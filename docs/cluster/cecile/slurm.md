...
=== "Array job and an alternative way"
    ...
    **Use case:** It is ideal when you need to run similar tasks multiple times. For example, the same analysis for many subjects, or you need to test different parameters for the same analysis.
    ...
    - Now you need to pass the subject ID paramenter to the script. To do that, you can then take advantage of the slurm variable `SLURM_ARRAY_TASK_ID`, which keeps track of the jobs count, by using it as an index. This means that for the first job of the array `SLURM_ARRAY_TASK_ID` will assume a value of 0 for the second job of 1 and so on until the last job. </b>  
